package mainGui;

import java.awt.Color;
import java.awt.Font;
import java.awt.Graphics;

@SuppressWarnings("serial")
public class TransperantButton extends CustomButton {

	private int		vx		= 0, vy = 0;
	private int		width	= 0, height = 0;
	// private String text;
	private Color	color;
	private int		factor;
	private int		fontsize;

	public TransperantButton(String text, int x, int y, int width, int height, int fontsize, int factor) {
		this.setXCoord(x);
		this.setYCoord(y);
		this.setWidthX(width);
		this.setHeightY(height);
		this.setText(text);
		this.setFontsize(fontsize);
		this.setFactor(factor);
		this.setBounds(x, y, width, height);
	}

	public void render(Graphics g) {
		g.setFont(new Font("Comic Sans MS", Font.PLAIN, fontsize));
		Color color = new Color(255, 0, 0, 255);
		Color color2 = new Color(1, 0, 0, 0);
		g.setColor(color);
		g.drawRect(vx, vy, width, height);
		g.setColor(color2);
		g.fillRect(vx + 1, vy + 1, width - 1, height - 1);
		g.setColor(Color.BLACK);
		g.drawString(this.getText(), this.vx + (width / 2) - (int) (g.getFontMetrics().getStringBounds(this.getText(), g).getWidth() / 2), this.vy + height * 3 / 4 + factor);
		if (this.getText() == "Lektionen")
			System.out.println("RENDER: " + this.getText() + " - X: " + this.getX() + " - Y: " + this.getY() + " - Width: " + this.getWidth() + " - Height: " + this.getHeight());
	}

	protected int getXCoord() {
		return vx;
	}

	protected void setXCoord(int x) {
		this.vx = x;
	}

	protected int getYCoord() {
		return vy;
	}

	protected void setYCoord(int y) {
		this.vy = y;
	}

	public int getWidthX() {
		return width;
	}

	protected void setWidthX(int width) {
		this.width = width;
	}

	protected int getHeightY() {
		return height;
	}

	protected void setHeightY(int height) {
		this.height = height;
	}

	protected Color getColor() {
		return color;
	}

	protected void setColor(Color color) {
		this.color = color;
	}

	protected int getFontsize() {
		return fontsize;
	}

	protected void setFontsize(int fontsize) {
		this.fontsize = fontsize;
	}

	protected int getFactor() {
		return factor;
	}

	protected void setFactor(int factor) {
		this.factor = factor;
	}

}
